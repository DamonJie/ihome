...
function changeheight() {
    ...
    if (bwidth<=768){
        ...
        houselist.css("padding","469px 0 0 0")
    }else{
        mapdiv.css("height",sheight-70);
        houselist.css("padding-top","70px")
    }
}
...
$(document).ready(function(){
    ...
    if (bwidth<=768) {
        ...
        mapdiv.css("height",sheight-70);
        houselist.css("padding-left","0","padding-right","0")
    }
    ...
    mapdiv.css("position","fixed");



    $(".house-list").on("mouseover",".house-item",function() {
        addr = $(this).find(".house-intro>em>em").html();

        $.ajax({
            url: "http://api.map.baidu.com/geocoder/v2/?address=" + addr + "&output=json&ak=To74dEGFgviL9rAI5ynLKOaPphh1BKIz",
            dataType:"jsonp",
            success:function (data) {
                map.panTo(new BMap.Point(data.result.location.lng, data.result.location.lat));
    ...
    customLayer.addEventListener('onhotspotclick',function(e)//单击热点图层
    {
