...
class AreaInfoHandler(BaseHandler):
    ...
    def get(self):
        ...
        if ret:
            ...
            resp = '{"errcode":"0", "errmsg":"OK", "data":%s}'%str(ret,encoding="utf-8")
            return self.write(resp)

        # 查询Mysql数据库，获取城区信息
        sql = "select ai_area_id,ai_name from ih_area_info;"
        try:
            ret = self.db.get_all(sql)
        except Exception as e:
            logging.error(e)
    ...
    def get(self):
        ...
        if ret:
            ...
            try:
                ...
                house_ret = self.db.get_all("select hi_house_id,hi_title,hi_order_count,hi_index_image_url from ih_house_info " \
            ...
            for l in house_ret:
                ...
                house = {
                    "house_id":l[0],
                    "title":l[1],
                    "img_url": constants.QINIU_URL_PREFIX + l[3]
                }
                houses.append(house)
        ...
        if ret:
            ...
            try:
                area_ret = self.db.get_all("select ai_area_id,ai_name from ih_area_info")
            except Exception as e:
                logging.error(e)
                area_ret = None
            areas = []
            if area_ret:
                for area in area_ret:
                    areas.append(dict(area_id=area[0], name=area[1]))
            json_areas = json.dumps(areas)
            try:
                self.redis.setex("area_info", constants.REDIS_AREA_INFO_EXPIRES_SECONDES, json_areas)
            except Exception as e:
                logging.error(e)
        resp = '{"errcode":"0", "errmsg":"OK", "houses":%s, "areas":%s}' % (str(json_houses,encoding="utf-8"), str(json_areas,encoding="utf-8"))
        self.write(resp)
...
class HouseInfoHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            ...
            self.db.insert(sql, (user_id, title, price, area_id, address,room_count, acreage, unit, capacity,beds, deposit, min_days, max_days))
            house_id =self.db.get_one('select * from ih_house_info order by hi_house_id desc')[0]
        except Exception as e:
            logging.error(e)
        ...
        try:
            ...
            logging.error(e)
            try:
                self.db.delete("delete from ih_house_info where hi_house_id=%s", (house_id,))
            except Exception as e:
                logging.error(e)
    ...
    def get(self):
        ...
        if ret:
            ...
            resp = '{"errcode":"0", "errmsg":"OK", "data":%s, "user_id":%s}' % (str(ret,encoding="utf-8"), user_id)
            return self.write(resp)
        ...
        try:
            ret = self.db.get_one(sql, house_id)
            print("ret",ret)
        except Exception as e:
            logging.error(e)
        ...
        data = {
            ...
            "max_days":ret[10],
            "user_name":ret[11],
            "user_avatar":constants.QINIU_URL_PREFIX + ret[12] if ret[12] else ""
        }

        # 查询房屋的图片信息
        sql = "select hi_url from ih_house_image where hi_house_id=%s"
        try:
            ret = self.db.get_all(sql, house_id)
        except Exception as e:
            logging.error(e)
            ret = None
        ...
        if ret:
            for image in ret:
                images.append(constants.QINIU_URL_PREFIX + image[0])
        data["images"] = images
...
class MyHouseHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            ret = self.db.get_all(sql, user_id)
        except Exception as e:
            logging.error(e)
        ...
        if ret:
            for l in ret:
                house = {
                    ...
                    "area_name": l[4],
                    "img_url": constants.QINIU_URL_PREFIX + l[5] if l[5] else ""
                }
                houses.append(house)
...
class HouseListRedisHandler(BaseHandler):
    ...
    def get(self):
        ...
        if start_date and end_date:
            sql_part = "((oi_begin_date>%s or oi_end_date<%s) " \
                       "or (oi_begin_date is null and oi_end_date is null))"
            sql_where.append(sql_part)
            sql_params+=(start_date,end_date,)
        elif start_date:
            sql_part = "(oi_end_date<%s or (oi_begin_date is null and oi_end_date is null))"
            sql_where.append(sql_part)
            sql_params+=(start_date,)
        elif end_date:
            sql_part = "(oi_begin_date>%s or (oi_begin_date is null and oi_end_date is null))"
            sql_where.append(sql_part)
            sql_params+=(end_date,)

        if area_id:
            sql_part = "hi_capacity=%s"
            sql_where.append(sql_part)
            sql_params+= (area_id,)
        ...
        # 有了where条件，先查询总条目数
        try:
            print(sql_total_count,sql_params)
            ret = self.db.get_one(sql_total_count)
        except Exception as e:
            logging.error(e)
            total_page = -1
        else:
            total_page = int(math.ceil(ret[0] / float(constants.HOUSE_LIST_PAGE_CAPACITY)))
            page = int(page)
            if page>total_page:
                return self.write(dict(errcode=RET.OK, errmsg="OK", data=[], total_page=total_page))
        ...
        # limit 20,3 从20条开始，返回3条数据
        if 1 == page:
            sql += " limit %s" % (str(constants.HOUSE_LIST_PAGE_CAPACITY * constants.HOUSE_LIST_PAGE_CACHE_NUM))
        else:
            sql += " limit %s,%s" % (str((page-1)*constants.HOUSE_LIST_PAGE_CAPACITY), str(constants.HOUSE_LIST_PAGE_CAPACITY * constants.HOUSE_LIST_PAGE_CACHE_NUM))

        logging.debug(sql)
        try:
            print("sql",sql)
            print(sql_params)
            ret = self.db.get_all(sql, sql_params)
        except Exception as e:
            logging.error(e)
            return self.write(dict(errcode=RET.DBERR, errmsg="查询出错"))
        data = []
        if ret:
            for l in ret:
                house = dict(
                    ...
                    address=l[4],
                    order_count=l[5],
                    avatar=constants.QINIU_URL_PREFIX+l[6] if l[6] else "",
                    image_url=constants.QINIU_URL_PREFIX+l[7] if l[7] else ""
                )
                data.append(house)
