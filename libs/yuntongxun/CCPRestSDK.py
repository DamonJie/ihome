...
class REST:
    ...
    def CreateSubAccount(self, friendlyName):
        ...
        sig = hashlib.new('md5',bytes(signature,encoding="utf-8")).hexdigest().upper()
        #拼接URL
    ...
    def sendTemplateSMS(self, to,datas,tempId):
        ...
        # h=hashlib.md5()
        # print(type(h))
        # sig = h.update(bytes(signature,encoding="utf-8"))
        # print(bytes(signature,encoding="utf-8"))
        # print(sig)
        ...
        #生成auth
        src = self.AccountSid + ":" + self.Batch;
        auth = base64.b64encode(bytes(src,encoding="utf-8")).strip()
        req = urllib2.Request(url)
        self.setHttpHeader(req)
        req.add_header("Authorization", auth)
        #创建包体
